  \usetikzlibrary {backgrounds,fit}
    \begin{tikzpicture}
      ...
      \draw (-3.7, -5.6) node (reinforce) {REINFORCE};
      \draw (-1.5, -5.75) node (mc-em) {MC-EM};
      \draw (-0.5, -5) node (reps) {REPS};
      \draw (1, -5.4) node (more) {MORE};      
      \draw [thick, dotted] (free_pg) to[out=-90,in=90] (reinforce);
      \draw [thick, dotted] (free_em) to[out=-90,in=90] (mc-em);
      \draw [thick, dotted] (free_inf) to[out=-90,in=90] (reps);
      \draw [thick, dotted] (free_inf) to[out=-90,in=90] (more);      

      
      \draw (4, -5.75) node (pegasus) {PEGASUS};
      \draw (6.5, -5) node (pilco) {$~~~~~~~$ PILCO $~~~~~~~$};
